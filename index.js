/* ═══════════════════════════════════════════════════════
   ELECTION ASSISTANT — index.js  v2.0
   Production-quality civic AI platform.
   Features: streaming responses, country context,
   Hindi toggle, WhatsApp share, offline FAQ fallback.
   No frameworks. Pure ES6+. ~500 lines.
═══════════════════════════════════════════════════════ */
...
const state = {
  ...
  chatHistory: [],
  activeTab:   'steps',
  country:     'India',   // active country context
  hindiMode:   false,     // respond in Hindi
};
...
Be neutral, factual, and encouraging of civic participation. Keep answers concise (under 250 words). Use plain paragraphs — no markdown asterisks. Be warm and educational.`,
...
Key topics you know deeply:
- Voter registration: Individual Electoral Registration (IER), gov.uk/register-to-vote, deadline (12 working days before polling), rolling registration
- First Past the Post (FPTP): UK uses FPTP for General Elections — candidate with most votes in a constituency wins, no majority needed
- Constituencies: 650 parliamentary constituencies (as of 2024 boundary review), each elects one MP
- General Elections: called by PM (max 5-year term under Fixed-term Parliaments Act repeal), dissolution of Parliament, King's Speech
- By-elections: held when an MP vacates their seat mid-term
- House of Commons vs. House of Lords: Commons is elected, Lords is appointed/hereditary
- Electoral Commission: independent body overseeing elections, registration, party finance
- Photo ID requirement: introduced 2023 — accepted IDs include passport, driving licence, Blue Badge, Oyster photocard (60+)
- Scottish Parliament, Welsh Senedd, Northern Ireland Assembly: use proportional representation (AMS / STV)
- Party political broadcasts: regulated by Ofcom, air time allocated by vote share
...
- Voter registration: importance, typical requirements (age, citizenship, ID, residence proof), online vs. in-person registration
- Election types: general, by-election, primary, runoff, referendum/plebiscite
- Voting systems: First Past the Post, Proportional Representation, Alternative Vote, Mixed Member Proportional, Single Transferable Vote
- Polling day: bringing ID, process at the polling station, secret ballot, accessibility for disabled voters
- Counting: hand counting vs. electronic, party observers, certification of results
- Campaign finance: spending limits, public funding, disclosure requirements
- Electoral bodies: typical roles of an independent election commission
...
- Common terms: constituency, mandate, turnout, spoilt ballot, recount, exit poll

If the user asks about a specific country, provide country-specific details where you know them. Be neutral, factual, and encouraging of civic participation. Keep answers concise (under 250 words). Use plain paragraphs. Be warm, approachable, and educational.`,
};
...
async function loadEnvKeys() {
  try {
    ...
    /* Silently fall through — offline/demo mode */
  }
}
...
function setupCountrySelector() {
  ...
  sel.addEventListener('change', () => {
    ...
    updateContextLabel();
    // Clear chat history when context changes so the AI starts fresh
    state.chatHistory = [];
...
function setupHindiToggle() {
  ...
  btn.addEventListener('click', () => {
    ...
    // Clear chat history so language change takes effect cleanly
    state.chatHistory = [];
    showToast(
      state.hindiMode ? 'हिंदी मोड सक्रिय — AI अब हिंदी में जवाब देगा' : 'Hindi mode off — responses in English',
      'info',
      2500
    );
  });
...
/**
 * Add a share button below a bot message bubble.
 * @param {HTMLElement} content  - the msg-content div
 * @param {string} text          - full response text to share
 */
function addShareButton(content, text) {
  ...
  shareBtn.setAttribute('title', 'Share via WhatsApp');
  shareBtn.innerHTML = `
...
function addUserMessage(text) {
  ...
  const meta = document.createElement('div');
  meta.className = 'msg-meta';
  meta.innerHTML = `<span class="msg-sender">You</span><span class="msg-time">${formatTime(new Date())}</span>`;
...
/** Safely render multi-paragraph text into a bubble element. */
...
async function sendMessage() {
  ...
  try {
    ...
    const messages = [
      ...
      { role: 'assistant', content: 'Hello! I\'m your Election Assistant. I\'m ready to help you understand elections, voter rights, and the democratic process. What would you like to know?' },
    ...
    if (!response.ok) {
      ...
      const errMsg  = errBody.error?.message || `HTTP ${response.status} — check your Groq API key`;
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          /* Replace typing indicator with streaming text on first token */
          if (!streamBubbleReady && loadingEl) {
            loadingEl.bubble.innerHTML = '';
            streamBubbleReady = true;
          }

          if (loadingEl) {
            renderTextToBubble(fullReply, loadingEl.bubble);
            scrollChat(box);
          }
        } catch (_) {
    ...
    if (loadingEl) {
      ...
      loadingEl.wrapper.setAttribute('aria-label', `AI error: ${err.message}`);
    }
...
/**
 * Share a pre-written FAQ answer.
 * Called from inline onclick in the FAQ HTML.
...
/**
 * Share arbitrary text via WhatsApp (or copy as fallback).
 * @param {string} text
 */
function shareText(text) {
  ...
  try {
    ...
    navigator.clipboard.writeText(formatted).then(() => {
      showToast('Copied to clipboard — paste in WhatsApp', 'info', 3000);
    }).catch(() => {
      showToast('Could not open WhatsApp or copy', 'error');
    });
  }
}
...
function buildMapQuery(loc) {
  ...
  return queries[state.mapType] || queries.booth;
}
